...
function buildUnknownDecision(context) {
  ...
  return {
    ...
    response: {
      whatIUnderstood:
        "Пока вижу только общее напряжение, но не вижу, в каком именно контуре системы оно живёт.",
      hypotheses: [
      ...
      whyItMatters:
        "Если сейчас сделать вид, что причина уже понятна, мы уйдём в красивую, но пустую диагностику.",
...
function buildProblemDecision(context) {
  ...
  if (hardSignal) {
    return {
      ...
      response: {
        whatIUnderstood:
          "Сигнал уже достаточно плотный: можно не крутиться вокруг симптома, а выбрать рабочее ограничение и первый ход.",
        hypotheses: [
          primaryConstraint,
          entryState.candidateConstraints[1]?.label || "Есть и соседняя версия, которую нужно держать в уме."
        ],
        whyItMatters:
          "Если менять самую рычажную точку системы, несколько симптомов начинают выравниваться одновременно.",
  ...
  return {
    ...
    response: {
      whatIUnderstood:
        "Вижу реальный симптом, но текущая причина пока звучит как версия пользователя, а не как доказанное ограничение системы.",
      hypotheses: [
        leadHypothesis,
        secondHypothesis
      ],
      whyItMatters:
        "Если схватиться за ближайшее объяснение, можно добавить людей, бюджет или процессы и только усилить шум вместо системного сдвига.",
