  ...
  if (/срыв|срок|ошибк|качество|исполн|доставк|delivery|заказ|обещани[ея]/i.test(normalized)) {
    return [
      ...
      buildConstraint(
        ...
        "Если обещание стандартное и реалистичное, а сбой только внутри delivery."
      ),
      buildConstraint(
...
function buildNextQuestion(focus, text, candidateConstraints, graphPacket) {
  ...
  if (/собственник|держится\s+на\s+мне|влезает|эскалац/i.test(normalized)) {
  ...
  if (/много\s+направ|распыл|нет\s+фокус|нет\s+стратег|непонятно.*выигрыв|слишком\s+много\s+продукт|всем\s+сразу/i.test(normalized)) {
    return "Если оставить только один рынок, один сегмент и один главный продукт на ближайшие 90 дней, что вы бы выбрали и от чего пришлось бы сознательно отказаться?";
  }
  ...
  if (focus === "profit") {
    return "Если разложить unit economics — экономику по каналу, сегменту и продукту, где ломается сильнее всего: цена, маржа, CAC, скидки или себестоимость выполнения?";
  }
