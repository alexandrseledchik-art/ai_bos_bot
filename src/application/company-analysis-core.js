...
function buildUpperFrameConstraint({ companyText, layerAnalyses }) {
  ...
  return {
    ...
    selectionBasis: [
      "Верхние слои задают рамку для всех решений ниже: собственник, рынок и стратегия.",
      "В данных есть сигналы по верхней рамке, но эталон ещё неполный.",
      "Нижние проблемы пока нельзя уверенно считать корнем: они могут быть следствием невыбранной игры."
    ],
    mode: "upper_frame",
    relatedLayers,
    evidence,
...
function selectConstraint({ company, sources, layerAnalyses }) {
  ...
  return {
    ...
    selectionBasis: [
      `У слоя "${primary.layerName}" самый сильный текущий сигнал среди собранных данных.`,
      primary.facts.length
        ? `Есть прямые факты по слою: ${primary.facts.slice(0, 2).join("; ")}.`
        : "Прямых фактов ещё мало, но слой лучше других связан с текущим запросом.",
      primary.missingFields.length
        ? `Главный пробел: не хватает эталона по ${primary.missingFields.slice(0, 3).join(", ")}.`
        : "Минимальный эталон слоя уже частично собран."
    ],
    rejectedAlternatives,
...
function buildNextStep(constraint) {
  ...
  if (constraint.mode === "upper_frame") {
    return {
      ...
      basis: [
        "Основная версия находится в верхней рамке: собственник, рынок и стратегия.",
        "Без этой рамки нельзя понять, какие финансы, процессы, роли и инструменты действительно нужны.",
        "Шаг даёт точку сравнения для всех нижних слоёв и не требует сразу перестраивать бизнес."
      ]
    };
  }
