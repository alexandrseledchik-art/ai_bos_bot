...
function buildRejectedAlternatives(layerAnalyses) {
  ...
  return ["finance", "operations", "team", "governance", "data_analytics", "technology", "commercial"]
    ...
    .map((item) => ({
      ...
      reason: `Область "${labels[item.layerCode] || item.layerName}" важна, но сейчас больше похожа на симптом или источник фактов. Сначала нужно понять цель собственника, рынок и стратегию.`
    }))
    .slice(0, 4);
}
...
const PROBLEM_TITLES_BY_LAYER = {
  owner_context: "Не до конца ясны цель и роль собственника",
...
function buildUpperFrameConstraint({ companyText, layerAnalyses }) {
  ...
  return {
    layer: "owner_context",
    layerName: "Цель, рынок и стратегия",
    title: "Неясны цель собственника, рынок и стратегия",
    explanation: [
      "Сейчас проблема выглядит не как поломка одной функции. Сначала нужно понять, что именно считается успехом для собственника, на каком рынке компания зарабатывает и за счёт чего собирается выигрывать.",
      "Пока это не прояснено, финансы, операции, команда и данные нельзя честно назвать корневой причиной: они могут быть следствиями неясного направления.",
    ...
    cause: "Пока не собраны вместе: цель собственника, горизонт, роль и ограничения плюс рыночные сегменты, спрос, маржа, конкуренция и отличие.",
    effects: [
    ...
    selectionBasis: [
      ...
      "По этим темам уже есть часть данных, но они пока не собраны в ясную рабочую картину.",
...
function buildNextStep(constraint) {
  ...
  if (constraint.mode === "upper_frame") {
    return {
      ...
      successCriteria: "Есть цель собственника и сегментная таблица, по которым можно выбрать 1-2 приоритетных клиентских потока для проверки.",
      basis: [
...
export class CompanyAnalysisCore {
  analyze({ state, companyId }) {
    ...
    const summary = probableConstraint.mode === "upper_frame"
      ? "Главная версия сейчас такая: нужно прояснить цель собственника, рынок и стратегию, чтобы не чинить симптомы вместо причины."
