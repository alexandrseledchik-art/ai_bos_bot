...
const TOOL_DISCOVERY_PATTERNS = [
  ...
  /дай\s+шаблон/i,
  /помоги\s+.*заполн/i,
  /заполн(ить|ением|ять)\s+.*(инструмент|шаблон|файл|таблиц|карт|bhag|бхаг)/i,
  /как\s+правильно\s+заполн/i,
  /чек-?лист/i,
  /таблиц[ау]/i
];
...
const SPECIFIC_TOOL_PATTERNS = [
  /\bbhag\b/i,
  /\bбхаг\b/i,
  /big\s+hairy\s+audacious\s+goal/i,
  /больш[а-яё\s-]+амбициозн[а-яё\s-]+цел/i,
  /дерзк[а-яё\s-]+цел/i,
  /\braci\b/i,
  /\bрас[иi]\b/i,
  /\bsipoc\b/i,
...
const META_ROLE_PATTERNS = [
  ...
  /как\s+ты\s+.*понимаешь\s+.*роль/i,
  /какая\s+у\s+тебя\s+роль/i,
  /кто\s+ты/i,
  /чем\s+(?:ты\s+)?можешь\s+помочь/i,
  /чем\s+(?:ты\s+)?поможешь/i,
  /можешь\s+(?:мне\s+)?помочь/i,
  /чем\s+(?:ты\s+)?можешь\s+быть\s+полез/i,
  /как\s+(?:ты\s+)?можешь\s+помочь/i,
  /что\s+ты\s+умеешь/i,
  /что\s+(?:ты\s+)?можешь(?:\s+делать)?/i,
  /(?:можно|умеешь|принимаешь|понимаешь|работаешь).*(?:голос|аудио|файл|документ|ссылк)/i,
  /(?:голос|аудио|файл|документ|ссылк).*(?:можно|умеешь|принимаешь|понимаешь|работаешь)/i,
  /в\s+каких\s+задачах/i,
  /как\s+с\s+тобой\s+работать/i,
  /что\s+ты\s+делаешь/i,
  /с\s+кем\s+.*отождествля/i,
  /с\s+чем\s+.*отождествля/i,
  /как\s+.*отождествля/i,
  /позициониру[её]шь/i,
  /позиционирован/i,
  /какая\s+у\s+тебя\s+позици/i,
  /11\s+сло[а-яё]+/i,
  /одиннадцат[а-яё\s]+сло/i,
  /по\s+сло[яеё]м/i
];
