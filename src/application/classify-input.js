...
export function classifyInput(text) {
  ...
  const hasOperationalPainSignal =
    /(ручн|перенос|дублир|crm|систем[аы]\s+не\s+связ|процесс|нет\s+метрик|не\s+видим|разные\s+цифр|отч[её]т|аналитик|непонятно[^а-яё0-9]+куда\s+движ|много\s+инициатив|мало\s+что\s+довод|не\s+повторя|не\s+тираж|нет\s+прибыл|прибыли\s+нет)/i.test(
      cleanText
    );
  const hasConcreteProblemSignal =
    (hasProblemMarkers || hasOperationalPainSignal) &&
    /(не\s+\S+|нет\s+(прибыл|денег|заяв|рост|фокус|стратег|метрик|цифр|продаж)|прибыли\s+нет|не хватает|не успева|не справля|перегруж|тонут|много|мало|просел|упал|падает|срыва|завис|долго|очеред|теря|дорог|буксу|ручн|перенос|дублир|спорят|непонятно|нет\s+нормальн|не\s+повторя|не\s+тираж|доводится)/i.test(
      cleanText
    );
  const looksVague =
