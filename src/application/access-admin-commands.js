...
export function buildAccessApprovedUserMessage(user = {}) {
  const name = String(user.first_name || user.firstName || "").trim();
  const greeting = name ? `Привет, ${name}. Доступ к AI-BOSS открыт.` : "Доступ к AI-BOSS открыт.";
  return [
    greeting,
    "",
    "AI-BOSS помогает последовательно собирать архитектуру бизнеса: удерживает контекст компании, ведёт по инструментам, сохраняет документы и помогает выбрать следующий управленческий шаг.",
    "",
    "Работать можно текстом, голосом, ссылками и файлами. Веб-кабинет показывает всё, что уже собрано, и текущий прогресс.",
    "",
    "Начнём здесь. Напиши одним сообщением:",
    "1. название компании;",
    "2. твою роль;",
    "3. что сейчас хочешь изменить или получить в результате.",
    "",
    "Если удобнее, сначала открой кабинет и заполни короткий профиль компании."
  ].join("\n");
}
