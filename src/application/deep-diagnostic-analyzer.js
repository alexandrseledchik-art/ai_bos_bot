...
function buildClassConclusion(classKey, avg, weakLayers) {
  ...
  if (classKey === "A") {
    return weakLayers.length
      ? `Нужно прояснить верхнюю рамку: ${weakNames}. Без этого нижние выводы могут чинить симптомы.`
      : "Условия игры выглядят достаточно понятными для следующего управленческого шага.";
  }
...
function buildWeakZones(weakestSubdomains = [], layerScores = []) {
  const weakLayers = layerScores
    ...
    .map((item) => ({
      ...
      title: item.layerName,
      score: item.score,
      why: "Низкая общая оценка слоя: зона может ограничивать систему или быть следствием более верхнего разрыва."
    }));
...
function selectRootHypothesis(layerScores) {
  ...
  if (weakUpper) {
    return {
      ...
      layer: "owner_context",
      confidence: "MEDIUM",
      why: "Слабая верхняя рамка лучше объясняет сразу несколько нижних просадок: финансы, операции, команда, управление и данные могут быть следствием невыбранной игры.",
...
function buildNextStep(rootHypothesis) {
  if (rootHypothesis.layer === "owner_context") {
    return {
      title: "Собрать рамку собственник-рынок и карту 5-7 сегментов.",
      why: "Это быстрее всего покажет, какую игру компания выбирает: где есть спрос, маржа, повторяемость, отличие и допустимый риск.",
      result: "Таблица сегментов и рамка собственника: цель, горизонт, роль, ограничения, риск."
    };
  }
  ...
  return {
    ...
    overall: {
      ...
      conclusion: rootHypothesis.layer === "owner_context"
        ? "Диагностика показывает не одну локальную поломку, а слабую верхнюю рамку: собственник, рынок и стратегия задают условия для остальных решений."
    ...
    consequences: rootHypothesis.layer === "owner_context"
      ? [
          ...
          "операции и команда могут строиться под невыбранную модель",
