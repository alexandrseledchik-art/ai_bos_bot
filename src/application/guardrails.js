...
function buildVagueSurfaceResponse(response, context) {
  ...
  if (isOpeningMessageContext(context) && language === "ru") {
    ...
    return joinParagraphs([
      `${greeting} Я помогаю смотреть на бизнес как на систему: найти главное ограничение, отделить симптом от причины и понять, что стоит делать первым. Здесь мы не заполняем анкету и не бросаемся советами вслепую.`,
