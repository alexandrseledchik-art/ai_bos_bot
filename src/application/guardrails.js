...
function explainBusinessTerms(text, context) {
  ...
  const replacements = [
    {
      ...
      replace: "профиль целевого клиента"
    },
    {
      test: /JTBD/i,
    ...
    {
      test: /\bSLA\b/i,
      alreadyExplained: /(SLA\s*[—-]|норматив\s+(по\s+)?скорости\s+(реакции|ответа))/i,
      replace: "норматив скорости ответа"
    },
    {
      test: /GTM/i,
  ...
  result = result
    .replace(/\bICP[-\s]?лид\b/gi, "целевой лид")
    .replace(/\bICP\b/gi, "профиль целевого клиента")
    .replace(/\bSLA\b/gi, "норматив скорости ответа")
    .replace(/\bGTM\b/gi, "модель выхода на рынок")
    .replace(/\bJTBD\b/gi, "задачу клиента, ради которой он покупает")
    .replace(/\bCAC\b/gi, "стоимость привлечения клиента")
    .replace(/\bdelivery\b/gi, "исполнение")
    .replace(/\bsales\b/gi, "продажи")
    .replace(/\bpipeline\b/gi, "воронку")
    .replace(/\bscorecard\b/gi, "карту критериев отбора")
...
function buildLeadScenarioField(spread, context, entryState) {
  ...
  if (strategicSplit) {
    return "Сейчас я бы уже держал не одну версию, а две системные развилки. Либо сам профиль целевого клиента и сегментация выбраны слишком широко, и рынок несёт вам лишний поток, либо сегмент в целом верный, но это не переведено в живые правила отбора, приоритета и передачи дальше.";
  }

  if (latestTextSuggestsWarmInbound(context)) {
    return "Тёплый поток отрезает только самую грубую версию про совсем случайный спрос. Но тёплый ещё не значит целевой. Для меня здесь остаются три версии: продавцы руками разбирают смешанный вход; профиль целевого клиента и приоритеты не переведены в живую обработку; или сам первый контур не держит закреплённую ответственность и очередь.";
  }

  if (latestTextLooksLikeLeadVolumeAndTiming(context) && hasSlowFirstResponse) {
    return "100 лидов в месяц на продавца и сутки до первого касания сами по себе ещё не объясняют корень. Для меня это скорее сигнал, что первый контур собран слабо: фильтрация, приоритет и закреплённая ответственность не держат живой поток как систему.";
  }
  ...
  if (latestTextSuggestsEarlyFunnelStage(context)) {
    return "Это уже переносит проблему в сам вход, а не в переговоры или дожим. Значит сейчас важнее не спорить о ресурсе, а отделить слабую фильтрацию потока от поломки закреплённой ответственности и первого отклика.";
  }
...
function buildLeadScenarioWhy(response, context, entryState) {
  if (strategicSplitNeeded(context, entryState)) {
    return "Одинаковые правила и похожая конверсия у команды ослабляют версию про конкретных людей. Значит сейчас важнее не спорить о менеджерах, а отделить ошибку в самой сегментации и профиле целевого клиента от ошибки перевода этих правил в живую коммерческую работу.";
  }
  ...
  if (isLeadFlowScenarioContext(context, entryState)) {
    return "Здесь опасно рано схлопнуться в локальную версию. Иначе легко принять ближайшую операционную боль за ограничение, которое на самом деле сидит в квалификации, сегментации или закреплённой ответственности.";
  }
...
function buildMetaWhySurfaceResponse(response, entryState, context) {
  ...
  const middle = isLeadFlowScenarioContext(context, entryState)
    ? strategicSplitNeeded(context, entryState)
      ? "Сейчас мне нужно не выбрать красивую историю, а развести две сильные стратегические версии: сам профиль целевого клиента и сегментация заданы неверно, или они в целом верны, но не превращены в рабочее правило отбора, приоритета и передачи дальше."
      ...
      : "Сначала мне нужно отделить локальный перегруз от двух других версий: в продавцов летит смешанный поток, или профиль целевого клиента и приоритеты вообще не доведены до живой обработки."
    : spread.length >= 3
...
function buildLeadScenarioSpread(context, entryState) {
  const spread = [
    {
      label: "Профиль целевого клиента или сегментация могут быть стратегически выбраны слишком широко, поэтому в систему изначально приходит лишний спрос",
      layer: "strategy",
      confidence: 0.63,
      ...
      whatWouldDisprove: "Если сегмент выбран узко и корректно, а проблема остаётся только в переводе правил в квалификацию и передачу дальше."
    },
    {
    ...
    {
      label: "Профиль целевого клиента, сегменты и правила приоритета не переведены из стратегии в живую коммерческую работу",
      layer: "strategy",
      confidence: 0.6,
      whyPossible: "Когда стратегия не превращена в правила отбора, квалификацию и маршрутизацию, продавцы забирают в работу всё подряд.",
      whatWouldDisprove: "Если сегменты и критерии приоритета уже жёстко работают в маркетинге, квалификации и передаче дальше."
    },
    {
      label: "Даже хорошие правила не собраны в устойчивый контур исполнения: не закреплены ответственность, передача дальше и контроль по входу",
      layer: "management",
      confidence: 0.56,
      whyPossible: "Даже при вменяемом профиле целевого клиента система может захлёбываться, если правило не превращено в управляемый контур с понятным владельцем и контролем исполнения.",
      whatWouldDisprove: "Если ответственность, передача дальше и контроль по входу уже жёстко закреплены и одинаково держатся без ручного вмешательства."
    },
    {
      label: "Первый отклик и закреплённая ответственность не держатся как устойчивая операционная конструкция",
      layer: "operations",
      confidence: 0.58,
      ...
      whatWouldDisprove: "Если закреплённая ответственность, норматив скорости ответа и маршрутизация уже работают стабильно по приоритетным лидам."
    },
    {
...
function pickBestNextQuestion(context, entryState, graphAnalysis) {
  ...
  if (isLeadFlowScenarioContext(context, entryState)) {
    ...
    if (latestTextSuggestsWarmInbound(context) && !systemicResolved) {
      return "Тёплый ещё не значит целевой. До продавца у вас есть слой квалификации и приоритета, который отделяет целевой лид от просто входящего интереса, или в работу идёт всё подряд?";
    }
    ...
    if (previousAssistantWasLocal && latestTextRestatesCapacityClaim(context) && !systemicResolved) {
      return hasWarmInbound
        ? "Скажу жёстче: продавцы у вас работают уже с отобранным тёплым целевым потоком, или они сначала сами руками отделяют живых клиентов от всего остального?"
...
function buildVagueSurfaceResponse(response, context) {
  ...
  if (isOpeningMessageContext(context) && language === "ru") {
    ...
    return joinParagraphs([
      `${greeting} Я помогаю смотреть на бизнес как на систему: где-то буду диагностом, где-то архитектором, где-то навигатором по инструментам или партнёром по следующему шагу. Роль выбираю по твоей задаче, а цель одна: найти главное ограничение, отделить симптом от причины и понять, что делать первым.`,
...
function buildHowToDefineIcpSurfaceResponse(entryState) {
  ...
  return joinParagraphs([
    "Профиль целевого клиента лучше определять не из головы и не красивым портретом, а по факту лучших сделок. Я бы взял последние 20-30 лидов, которые быстрее всего дошли до денег, и посмотрел, что у них повторяется: сегмент, размер, задача, бюджет, срочность и кто принимает решение.",
...
function buildWhatIsIcpSurfaceResponse(entryState) {
  ...
  return joinParagraphs([
    "Если совсем по-простому, профиль целевого клиента — это правило, кто для вас свой клиент, а кто только создаёт шум на входе. Не портрет «идеального клиента», а рабочий фильтр: кому даём приоритет, а кого должны отрезать раньше.",
...
function buildDirectionSurfaceResponse(response, entryState) {
  ...
  if (ensureArray(entryState?.candidateConstraints, 6).some((item) => /icp|сегментац/i.test(ensureString(item?.label))) &&
    ...
    return joinParagraphs([
      ...
      "Либо сам профиль целевого клиента и сегментация выбраны неверно, и вы кормите систему лишним потоком ещё на входе. Либо сегмент в целом верный, но он не доведён до рабочих правил отбора, приоритета и передачи дальше.",
      nextQuestion
    ]);
  }
...
function buildNextSurfaceResponse(response, entryState, context) {
  ...
  if (isLeadFlowScenarioContext(context, entryState) && strategicSplitNeeded(context, entryState)) {
    return joinParagraphs([
      ...
      "Сначала отделю ошибку в самой сегментации и профиле целевого клиента от ошибки перевода этих правил в живую работу команды. После этого уже станет видно, мы лечим стратегию входа или конструкцию первого контура.",
      nextQuestion
    ]);
  }
...
function buildRoadmapSurfaceResponse(response, entryState, context) {
  ...
  if (isLeadFlowScenarioContext(context, entryState) && strategicSplitNeeded(context, entryState)) {
    return joinParagraphs([
      ...
      "Первая версия: сам профиль целевого клиента и сегментация выбраны слишком широко, поэтому лишний поток создаётся ещё до квалификации. Вторая версия: сегмент в целом верный, но он не доведён до живых правил отбора, приоритета и передачи дальше, поэтому команда всё равно тонет на входе.",
  ...
  if (isLeadFlowScenarioContext(context, entryState)) {
    return joinParagraphs([
      ...
      "Если поток смешанный или плохо отфильтрован, копаем в сегментацию, квалификацию и приоритет. Если поток уже чистый, а вход всё равно ломается, тогда смотрим закреплённую ответственность, очередь и уже потом мощность.",
      nextQuestion
    ]);
  }
  ...
  if (/заяв|лид/.test(text) && /не усп|люд|ответ|очеред|обработ|продавц|менеджер|штат/.test(text)) {
    constraints.push(
      {
        ...
        layer: "commercial",
        confidence: 0.58,
        whyPossible: "Большой входящий поток может создавать ложное ощущение, что проблема только в ресурсе, если значимая доля лидов не соответствует профилю целевого клиента.",
      ...
      },
      {
        ...
        layer: "management",
        confidence: 0.57,
        whyPossible: "Даже хорошие лиды теряются, когда первый ответ лежит в общей очереди без владельца и норматива скорости ответа.",
        whatWouldDisprove: "Если входящие уже распределяются по роли, норматив скорости ответа соблюдается, а очередь всё равно растёт."
      },
      {
        label: "Продавцы тащат на себе разбор, квалификацию и сортировку входа вместо продажи",
        layer: "commercial",
    ...
    constraints.push(
      ...
      },
      {
        ...
        layer: "commercial",
        confidence: 0.5,
        whyPossible: "Слабый профиль целевого клиента или квалификация могут делать продажи плохими при внешне нормальном объёме входящего потока.",
        whatWouldDisprove: "Если большинство входящих лидов целевые и соответствуют профилю целевого клиента."
      }
    );
  } else if (/прибыл|марж|касс|деньг/.test(text)) {
    constraints.push(
      {
        label: "Экономика ломается в юнит-экономике, то есть экономике одной сделки, клиента или канала, а не в объёме продаж",
        layer: "finance",
        confidence: 0.57,
        whyPossible: "Падение прибыли часто связано с качеством спроса, скидками, себестоимостью или стоимостью привлечения клиента, а не только с объёмом.",
        whatWouldDisprove: "Если экономика по каналам и продуктам уже прозрачна и не показывает утечки."
      },
      {
...
export function applyGuardrails(rawDecision, context) {
  ...
  if (shouldHoldLeadFlowInClarify(context, decision.entryState)) {
    ...
    decision.response.whatIUnderstood = strategicSplitNeeded(context, decision.entryState)
      ? "Теперь уже видно не одну, а две системные развилки: либо сам профиль целевого клиента и сегментация заданы неверно, либо они в целом верны, но не превращены в живые правила отбора и маршрута."
      : latestTextSuggestsWarmInbound(context)
    ...
    decision.response.whyItMatters = strategicSplitNeeded(context, decision.entryState)
      ...
      : "Если сейчас схлопнуться в версию про найм или норматив скорости ответа, можно пропустить более глубокую поломку в квалификации, сегментации и конструкции первого контура.";
  }
