...
function buildVagueSurfaceResponse(response, context) {
  ...
  if (isOpeningMessageContext(context) && language === "ru") {
    ...
    return joinParagraphs([
      `${greeting} Я бизнес-диагност: помогаю найти главное ограничение, отделить симптом от причины и понять, что делать первым. Здесь мы не заполняем анкету и не бросаемся советами вслепую.`,
