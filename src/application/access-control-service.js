...
export function buildAccessDeniedReply(decision) {
  ...
  return [
    "Заявка на доступ к AI-BOSS отправлена.",
    "Доступ подтверждает администратор, чтобы в рабочих кабинетах не появлялись случайные пользователи и боты.",
    "После одобрения я помогу создать контекст компании и начать работу: можно будет писать текстом, присылать голосовые, ссылки и файлы."
  ].join("\n\n");
}
