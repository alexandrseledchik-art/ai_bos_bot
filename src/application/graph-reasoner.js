...
function buildQuestionCandidates({ candidateStates, candidateCauses, observedSignals, extracted }) {
  ...
  if (leadFlowScenario && strategicSplit) {
    candidates.push({
      question:
        "Чтобы развести стратегическую ошибку и ошибку перевода в процесс, мне нужен факт: где у вас уже жёстко закреплены правила отбора и приоритета — в рекламе, в квалификации, в маршрутизации дальше, а где команда до сих пор решает это вручную?",
      type: "cause",
      layer: "strategy",
