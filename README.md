...
## Управленческий цикл в Telegram

Когда бот сформировал рабочую гипотезу и один проверочный шаг, цикл можно вести прямо в обычном Telegram-чате:

- `фиксируем` — принять гипотезу и шаг как текущее управленческое решение;
- `не фиксируем` — отклонить предложенную версию;
- `статус решения` — показать активную гипотезу, шаг и контрольную дату;
- `готово` — сообщить о выполнении шага;
- `результат: ...` — сохранить наблюдаемый эффект и закрыть цикл.

Обычное `да` намеренно не считается принятием решения: бот просит явную команду `фиксируем`. Telegram Mini App для этого контура не требуется.

## Профиль аудитории без смешения осей

AI-BOSS постепенно хранит отдельно роль человека, размер бизнеса, стадию развития, состояние бизнеса, отрасль, канал входа и текущий режим работы. Задача относится к активному кейсу. Сегмент подтверждается только по пересечению нескольких осей; книга, QR или Telegram остаются каналами входа и сами по себе сегментом не считаются.

Первое коммерческое ядро — собственник выросшего бизнеса с разрывом управляемости. Техническая модель находится в `src/domain/audience-segmentation.js`.
